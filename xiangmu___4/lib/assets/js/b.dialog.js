...
}(window.top.jQuery.fn.modal && typeof(window.top.jQuery.fn.modal.Constructor.VERSION) == 'string' ? jQuery : window.top.jQuery);;
(function($) {
	"use strict";
	//弹出窗口模板
	var _template = '<div class="modal hide bDialog" dialog="bDialog" tabindex="-1" aria-labelledby="bDialogHeaderLabel" role="dialog" aria-hidden="true">' +
		...
		'<div class="modal-header bDialogHeader bg-white pad10">' +
		'<button type="button" class="close bDialogCloseButton float-r" data-dismiss="modal" aria-hidden="true"><i class="iconfont icon-close"></i></button>' +
	...
	var _bDialog = {
		...
		atLast: function(dialog, p) {
			...
			//处理窗口拖拽
			if(p.drag && $.fn.draggable && !msie && !p.fullWidth) $(dialog).draggable({
				handle: "div.bDialogHeader"
			});
		},
		/**
		 * 最大化窗口
